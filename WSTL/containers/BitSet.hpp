...
namespace WSTL
{
    ...
    {
    public:
        template <class StringElement, class StringTraits = std::char_traits<StringElement>, class StringAlloc =
                  std::allocator<StringElement>>
        using String = std::basic_string<StringElement, StringTraits, StringAlloc>;
        typedef ConditionalT<BitAmount <= sizeof(unsigned long) * CHAR_BIT, unsigned long, unsigned long long>
        ArrayType;
        ...
        constexpr void Construct(const Elem* const ptr, ::Size count, const Elem zero, const Elem one)
        {
            if (count > BitAmount)
            {
                ...
                {
                    const auto c = ptr[i];
                    if (!Traits::eq(one, c) && !Traits::eq(zero, c))
                        throw std::invalid_argument(
                            "Invalid Bitset Character");
                }
        ...
        constexpr explicit BitSet(const String<Elem, Traits, Alloc>& str,
                                  String<Elem, Traits, Alloc>::size_type pos = 0,
                                  String<Elem, Traits, Alloc>::size_type count = String<
                                      Elem, Traits, Alloc>::npos,
        ...
        constexpr explicit BitSet(const Elem* str,
                                  String<Elem>::size_type count = String<Elem>::npos,
                                  Elem zero = static_cast<Elem>('0'),
                                  Elem one = static_cast<Elem>('1'))
        {
